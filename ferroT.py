...
def mag(m, U, n, nnn, T):
    ...
    mu = diag.good_order2(list(ew1)+list(ew2))[int(NN*n)]#Fermienergy
    for d in state1:
        nn = [nn[x]+np.abs(d[1][x])**2*fermi(T, mu, d[0]) for x in range(NNN)]
        ma = [ma[x]+np.abs(d[1][x])**2*fermi(T, mu, d[0]) for x in range(NNN)]
...
def m_T(n, U):
    ...
    for q in range(3):
        m[0].append(0.01)
        x = ferro_mag(U, n,[n_right(n)*(1)**(i not in ind) for i in range(NNN)], [n for i in range(NNN)],0.01)
        m[1].append(x[0])#[n_right(n)*(1)**(i not in ind) for i in range(NNN)])
        nn.append(x[1])
        en.append(x[2])
    for T in [x/100000 for x in range(1000, 300000, 500)]:
        m[0].append(T)
        x = ferro_mag(U, n, mixing([n_right(n)*(1)**(i not in ind) for i in range(NNN)], m[1][-1], m[1][-2],m[1][-3]), mixing([n for i in range(NNN)], nn[-1], nn[-2],nn[-3]),T)
...
for U in [10]:
    print(U)
    m = m_T(N,U)
#    fobj = open("C:\\Users\\Ch\\Desktop\\uni\\6. Semester\\BA Störstelle\\FerroT\\fs12_"+str(N)+".dat", "w")
#    fobj_en = open("C:\\Users\\Ch\\Desktop\\uni\\6. Semester\\BA Störstelle\\FerroT\\energie_fs12_"+str(N)+".dat", "w")
